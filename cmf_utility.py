...
# X_H is either Mg_H or Si_H depending on method
def Abundance_to_CMF(Fe_H, X_H, method="Fe2Mg"):
    A_Fe_Sun=7.46-12                        #log10(n(Fe)/n(H))
    Met=Fe_H                          #Metallicity [Fe/H]
    Fe_ab=(10**(Met+(A_Fe_Sun)))            #Fe/H absoluteat abundance
    M_Fe=Fe_ab*55.845    #Mass of Fe/ Mass of H
    
    A_X_Sun=7.55-12
    X_ab=(10**(X_H+(A_X_Sun)))
    M_X=X_ab*24.305                        #Mass of X / Mass of H
    FeXStar=M_Fe/M_X
    cmf=st_pl(FeXStar,si=0,fe=0.1,py=0.6,method=method,f_FeX=None)

    return(FeXStar, cmf)

# Calculates the CMF using precomputed results from Abundance_to_CMF and polynomial fit
def Fast_Abundance_CMF(Fe_H, X_H, method="Fe2Mg"):
    coeffs = Mg_coeffs if method == "Fe2Mg" else Si_coeffs
    return np.polynomial.polynomial.polyval(Fe_H - X_H, coeffs)